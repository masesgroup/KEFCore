...
/// </summary>
public class KEFCoreTable<TKey, TValueContainer, TJVMKey, TJVMValueContainer> : IKEFCoreTable
    where TKey : notnull
    where TValueContainer : class, IValueContainer<TKey>
{
    ...
    {
        Database = database;
        Database.InfrastructureLogger.CheckAndLogDebug("KEFCoreTable Creating new KafkaTable for {Name}", entityType.Name);
        ...
        var ctor = typeof(KEFCoreRowBag<TKey>).GetConstructors().First(); // ([typeof(IUpdateEntry), typeof(string), typeof(TKey), typeof(IProperty[]), typeof(object?[]), typeof(IComplexProperty[]), typeof(object?[])])!;
    ...
    {
        Database.InfrastructureLogger.CheckAndLogDebug("KEFCoreTable::Dispose for {Name}", EntityType.Name);
        _producer.Dispose();
    }
    /// <inheritdoc/>
    ...
    public virtual IKEFCoreRowBag Create(IUpdateEntry entry)
    {
        ...
        {
            throw new DbUpdateConcurrencyException(KEFCoreStrings.DuplicateKeyException(key), [entry]);
        }
        else
        {
            ...
            for (int index = 0; index < flattenedProperties.Length; index++)
            {
                ...
                KEFCoreTable<TKey, TValueContainer, TJVMKey, TJVMValueContainer>.HasNullabilityError(flattenedProperties[index], propertyValue, nullabilityErrors);
            }
    ...
    public virtual IKEFCoreRowBag Delete(IUpdateEntry entry)
    {
        ...
        {
            throw new DbUpdateConcurrencyException(KEFCoreStrings.UpdateConcurrencyException(key), [entry]);
        }
        else
        {
            var properties = _propertiesCache.GetOrAdd(entry.EntityType, static (type) => [.. type.GetProperties()]);
    ...
    public virtual IKEFCoreRowBag Update(IUpdateEntry entry)
    {
        ...
        {
            throw new DbUpdateConcurrencyException(KEFCoreStrings.UpdateConcurrencyException(key), new[] { entry });
        }
        else
        {
            var properties = _propertiesCache.GetOrAdd(entry.EntityType, static (type) => [.. type.GetProperties()]);
            ...
            for (int index = 0; index < flattenedProperties.Length; index++)
            {
                ...
                if (KEFCoreTable<TKey, TValueContainer, TJVMKey, TJVMValueContainer>.HasNullabilityError(flattenedProperties[index], data[index], nullabilityErrors))
                {
                    continue;
                }
        ...
        IList<IProperty> nullabilityErrors)
    {
        if (_loggingOptions.IsSensitiveDataLoggingEnabled)
        {
            throw new DbUpdateException(
                KEFCoreStrings.NullabilityErrorExceptionSensitive(
        ...
        throw new DbUpdateException(
            KEFCoreStrings.NullabilityErrorException(
                nullabilityErrors.Format(),
                entry.EntityType.DisplayName()),
            [entry]);
    }
        ...
        Dictionary<IProperty, object?> concurrencyConflicts)
    {
        if (_loggingOptions.IsSensitiveDataLoggingEnabled)
        {
            throw new DbUpdateConcurrencyException(
                KEFCoreStrings.UpdateConcurrencyTokenExceptionSensitive(
                    entry.EntityType.DisplayName(),
                    entry.BuildCurrentValuesString(entry.EntityType.FindPrimaryKey()!.Properties),
        ...
        throw new DbUpdateConcurrencyException(
            KEFCoreStrings.UpdateConcurrencyTokenException(
                entry.EntityType.DisplayName(),
