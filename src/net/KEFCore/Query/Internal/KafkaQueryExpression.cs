...
public partial class KafkaQueryExpression : Expression, IPrintableExpression
{
        ...
        if (_clientProjections.Count > 0)
        {
            for (var i = 0; i < _clientProjections.Count; i++)
            {
                ...
                switch (projection)
                {
                    ...
                    case KafkaQueryExpression kafkaQueryExpression:
                    {
    ...
    public virtual void ApplySetOperation(MethodInfo setOperationMethodInfo, KafkaQueryExpression source2)
    {
        ...
        {
            throw new InvalidOperationException(KafkaStrings.SetOperationsNotAllowedAfterClientEvaluation);
        }
    ...
    public virtual void ApplyDistinct()
    {
        ...
        else
        {
            ...
            {
                var projection = _clientProjections[i];
                if (projection is KafkaQueryExpression)
                {
                    throw new InvalidOperationException(KafkaStrings.DistinctOnSubqueryNotSupported);
                }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual Expression AddRightJoin(
        KafkaQueryExpression innerQueryExpression,
        LambdaExpression outerKeySelector,
        LambdaExpression innerKeySelector,
        Expression outerShaperExpression,
    ...
    private KafkaQueryExpression Clone()
    {
        _cloningExpressionVisitor ??= new CloningExpressionVisitor();

        return (KafkaQueryExpression)_cloningExpressionVisitor.Visit(this);
    }
