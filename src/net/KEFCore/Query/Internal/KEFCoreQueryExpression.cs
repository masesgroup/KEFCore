...
/// </summary>
public partial class KEFCoreQueryExpression : Expression, IPrintableExpression
{
    ...
    private static readonly MethodInfo LeftJoinMethodInfo = typeof(KEFCoreQueryExpression).GetTypeInfo()
    ...
    private KEFCoreQueryExpression(
    ...
    public KEFCoreQueryExpression(IEntityType entityType)
    {
        ...
        ServerQueryExpression = new KEFCoreTableExpression(entityType);
    ...
    public virtual void ApplySetOperation(MethodInfo setOperationMethodInfo, KEFCoreQueryExpression source2)
    {
        ...
        {
            throw new InvalidOperationException(KEFCoreStrings.SetOperationsNotAllowedAfterClientEvaluation);
        }
    ...
    public virtual void ApplyDefaultIfEmpty()
    {
        if (_clientProjections.Count != 0)
        {
            throw new InvalidOperationException(KEFCoreStrings.DefaultIfEmptyAppliedAfterProjection);
        }
    ...
    public virtual void ApplyDistinct()
    {
        ...
        else
        {
            ...
            {
                var projection = _clientProjections[i];
                if (projection is KEFCoreQueryExpression)
                {
                    throw new InvalidOperationException(KEFCoreStrings.DistinctOnSubqueryNotSupported);
                }
    ...
    public virtual Expression AddInnerJoin(
        KEFCoreQueryExpression innerQueryExpression,
    ...
    {
        expressionPrinter.AppendLine(nameof(KEFCoreQueryExpression) + ": ");
    ...
    private KEFCoreQueryExpression Clone()
    {
        _cloningExpressionVisitor ??= new CloningExpressionVisitor();

        return (KEFCoreQueryExpression)_cloningExpressionVisitor.Visit(this);
    }
        ...
        switch (key)
        {
            ...
            case StructuralTypeShaperExpression { ValueBufferExpression: ProjectionBindingExpression projectionBindingExpression } shaper:
                var entityProjectionExpression =
                    (EntityProjectionExpression)((KEFCoreQueryExpression)projectionBindingExpression.QueryExpression)
