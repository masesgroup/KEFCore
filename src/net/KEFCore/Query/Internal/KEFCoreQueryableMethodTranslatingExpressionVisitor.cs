...
/// </summary>
public class KEFCoreQueryableMethodTranslatingExpressionVisitor : QueryableMethodTranslatingExpressionVisitor
{
    private readonly KEFCoreExpressionTranslatingExpressionVisitor _expressionTranslator;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public KEFCoreQueryableMethodTranslatingExpressionVisitor(
        QueryableMethodTranslatingExpressionVisitorDependencies dependencies,
        QueryCompilationContext queryCompilationContext)
        : base(dependencies, queryCompilationContext, subquery: false)
    {
        _expressionTranslator = new KEFCoreExpressionTranslatingExpressionVisitor(queryCompilationContext, this);
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected KEFCoreQueryableMethodTranslatingExpressionVisitor(
        KEFCoreQueryableMethodTranslatingExpressionVisitor parentVisitor)
        : base(parentVisitor.Dependencies, parentVisitor.QueryCompilationContext, subquery: true)
    {
        _expressionTranslator = new KEFCoreExpressionTranslatingExpressionVisitor(QueryCompilationContext, parentVisitor);
        _weakEntityExpandingExpressionVisitor = new SharedTypeEntityExpandingExpressionVisitor(_expressionTranslator);
        _projectionBindingExpressionVisitor = new KEFCoreProjectionBindingExpressionVisitor(this, _expressionTranslator);
    ...
    protected override QueryableMethodTranslatingExpressionVisitor CreateSubqueryVisitor()
        => new KEFCoreQueryableMethodTranslatingExpressionVisitor(this);
    ...
    protected override Expression VisitExtension(Expression extensionExpression)
    {
        switch (extensionExpression)
        {
            case GroupByShaperExpression groupByShaperExpression:
                ...
                return ((KEFCoreQueryExpression)groupShapedQueryExpression.QueryExpression)
                    .Clone(groupShapedQueryExpression.ShaperExpression);

            case ShapedQueryExpression shapedQueryExpression:
                return ((KEFCoreQueryExpression)shapedQueryExpression.QueryExpression)
    ...
    {
        var queryExpression = new KEFCoreQueryExpression(entityType);
    ...
    protected override ShapedQueryExpression? TranslateDefaultIfEmpty(ShapedQueryExpression source, Expression? defaultValue)
    {
        ...
        {
            ((KEFCoreQueryExpression)source.QueryExpression).ApplyDefaultIfEmpty();
    ...
    {
        ((KEFCoreQueryExpression)source.QueryExpression).ApplyDistinct();
        ...
        LambdaExpression resultSelector)
    {
        ...
        var outerShaperExpression = ((KEFCoreQueryExpression)outer.QueryExpression).AddInnerJoin(
            (KEFCoreQueryExpression)inner.QueryExpression,
            outerKeySelector,
            innerKeySelector,
        ...
        LambdaExpression innerKeySelector)
    {
        ...
        outerKeySelector = Expression.Lambda(outerKeyBody, ((KEFCoreQueryExpression)outer.QueryExpression).CurrentParameter);
        innerKeySelector = Expression.Lambda(innerKeyBody, ((KEFCoreQueryExpression)inner.QueryExpression).CurrentParameter);
        ...
        bool returnDefault)
    {
        ...
        switch (kefcoreQueryExpression.ServerQueryExpression)
        {
            ...
            case KEFCoreSingleKeyTableExpression:
                break;
        ...
        LambdaExpression resultSelector)
    {
        ...
        var outerShaperExpression = ((KEFCoreQueryExpression)outer.QueryExpression).AddLeftJoin(
            (KEFCoreQueryExpression)inner.QueryExpression,
            outerKeySelector,
            innerKeySelector,
            outer.ShaperExpression,
...
#warning to be rewieved
    ...
    protected override ShapedQueryExpression TranslateSelect(ShapedQueryExpression source, LambdaExpression selector)
    {
        ...
        var queryExpression = (KEFCoreQueryExpression)source.QueryExpression;
        ...
        LambdaExpression resultSelector)
    {
        ...
        {
            var outerShaperExpression = ((KEFCoreQueryExpression)source.QueryExpression).AddSelectMany(
                (KEFCoreQueryExpression)inner.QueryExpression, source.ShaperExpression, inner.ShaperExpression, defaultIfEmpty);
        ...
        bool preserveType = false)
    {
        ...
        return lambdaBody != null
            ? Expression.Lambda(
                lambdaBody,
                ((KEFCoreQueryExpression)shapedQueryExpression.QueryExpression).CurrentParameter)
            : null;
    }
    ...
    private Expression RemapLambdaBody(ShapedQueryExpression shapedQueryExpression, LambdaExpression lambdaExpression)
    {
        ...
        return ExpandSharedTypeEntities((KEFCoreQueryExpression)shapedQueryExpression.QueryExpression, lambdaBody);
    }

    private Expression ExpandSharedTypeEntities(KEFCoreQueryExpression queryExpression, Expression lambdaBody)
        => _weakEntityExpandingExpressionVisitor.Expand(queryExpression, lambdaBody);

    private sealed class SharedTypeEntityExpandingExpressionVisitor(KEFCoreExpressionTranslatingExpressionVisitor expressionTranslator)
        : ExpressionVisitor
    {
        private KEFCoreQueryExpression _queryExpression = null!;

        public string? TranslationErrorDetails
            => expressionTranslator.TranslationErrorDetails;

        public Expression Expand(KEFCoreQueryExpression queryExpression, Expression lambdaBody)
        {
        ...
        private Expression? TryExpand(Expression? source, MemberIdentity member)
        {
            ...
            if (navigation.IsCollection)
            {
                ...
                var innerQueryExpression = (KEFCoreQueryExpression)innerShapedQuery.QueryExpression;
