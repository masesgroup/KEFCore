...
public partial class AvroValueContainer<TKey> : AvroValueContainer, IValueContainer<TKey> where TKey : notnull
{
    ...
    public AvroValueContainer(IValueContainerData valueContainerData, IComplexTypeConverterFactory? complexTypeFactory = null)
    {
        ...
        for (int i = 0; i < properties.Length; i++)
        {
            ...
            var pRecord = new PropertyDataRecord
            {
                ...
                ClrType = _type.Item1 == WellKnownManagedTypes.Undefined ? item.ClrType?.ToAssemblyQualified() : null,
    ...
    static void BuildPropertyValue(IPropertyBase? property, bool supportNull, ref WellKnownManagedTypes managedType, ref object? inputValue, IComplexTypeConverterFactory? complexTypeFactory = null)
    {
        switch (managedType)
        {
            ...
            case WellKnownManagedTypes.DateTimeOffset:
                ...
                break;
            case WellKnownManagedTypes.Char:
                ...
                break;
            case WellKnownManagedTypes.SByte:
                ...
                break;
            case WellKnownManagedTypes.UShort:
                ...
                break;
            case WellKnownManagedTypes.UInt:
                ...
                break;
            case WellKnownManagedTypes.ULong:
                ...
                break;
            case WellKnownManagedTypes.Decimal:
    ...
    {
        switch ((WellKnownManagedTypes)record.ManagedType)
        {
            case WellKnownManagedTypes.Guid:
                {
                    if (record.Value != null)
                    {
                        if (!Guid.TryParse(record.Value as string, out Guid guid)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(Guid)}");
                        input = guid;
                    }
            ...
            case WellKnownManagedTypes.DateTime:
                {
                    if (record.Value != null)
                    {
                        if (!DateTime.TryParse(record.Value as string, out DateTime dt)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(DateTime)}");
                        input = dt;
                    }
            ...
            case WellKnownManagedTypes.DateTimeOffset:
                {
                    if (record.Value != null)
                    {
                        if (!DateTimeOffset.TryParse(record.Value as string, out DateTimeOffset dto)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(DateTimeOffset)}");
                        input = dto;
                    }
            ...
            case WellKnownManagedTypes.Char:
                {
                    if (record.Value != null)
                    {
                        if (!char.TryParse(record.Value as string, out char dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.Char)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.SByte:
                {
                    if (record.Value != null)
                    {
                        if (!sbyte.TryParse(record.Value as string, out sbyte dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.SByte)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.UShort:
                {
                    if (record.Value != null)
                    {
                        if (!ushort.TryParse(record.Value as string, out ushort dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.UInt16)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.UInt:
                {
                    if (record.Value != null)
                    {
                        if (!uint.TryParse(record.Value as string, out uint dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.UInt32)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.ULong:
                {
                    if (record.Value != null)
                    {
                        if (!ulong.TryParse(record.Value as string, out ulong dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.UInt64)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.Decimal:
                {
                    if (record.Value != null)
                    {
                        if (!decimal.TryParse(record.Value as string, out decimal dec)) throw new InvalidCastException($"Unable to convert {record.Value} (property '{record.PropertyName}', managed type {(WellKnownManagedTypes)record.ManagedType}) into {nameof(System.Decimal)}");
                        input = dec;
                    }
            ...
            case WellKnownManagedTypes.ComplexType:
                {
                    ...
                    else throw new InvalidOperationException($"Avro ComplexType cannot manage input {value} for type {(property != null ? property.ClrType : record.ClrType)} without a proper {nameof(IComplexTypeConverterFactory)} instance.");
                    input = value;
                }
                break;
            case WellKnownManagedTypes.ComplexTypeAsJson:
                {
                    object value = record.Value;
                    if (value == null)
                    {
                    ...
                    {
                        Type type = property?.ClrType ?? Type.GetType(record.ClrType)!;
                        input = JsonSupport.ValueContainer.Deserialize(type, str);
                    }
                    else throw new InvalidCastException($"Cannot convert record value {value} into string.");
                }
                break;
